...
from book_recommendations import telemetry as t


def test_telemetry_disabled_flags(monkeypatch):
    for flag, value in (
        ("BOOK_RECOMMENDATIONS_TELEMETRY", "false"),
        ("BOOK_RECOMMENDATIONS_TELEMETRY", "0"),
        ("DISABLE_TELEMETRY", "1"),
        ("DO_NOT_TRACK", "true"),
        ("NO_TELEMETRY", "on"),
    ):
...
def test_telemetry_enabled_by_default(monkeypatch):
    monkeypatch.delenv("BOOK_RECOMMENDATIONS_TELEMETRY", raising=False)
...
def test_opt_out_never_writes_identity(monkeypatch, tmp_path):
    ...
    assert not (tmp_path / ".book_recommendations").exists()
