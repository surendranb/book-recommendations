...
GATEWAY_URLS = [
    "https://book-recommendations.builditwithai.xyz/e",
    # .workers.dev fallback: prevents event loss during DNS propagation or
    # edge routing blips (fleet dual-endpoint standard).
    "https://book-recommendations-install-telemetry.reachsuren.workers.dev/e",
]
_env_url = os.getenv("BOOK_RECOMMENDATIONS_TELEMETRY_URL")
...
try:
    ...
    MCP_SERVER_VERSION = importlib.metadata.version("book-recommendations")
except Exception:
    MCP_SERVER_VERSION = "unknown"


# Any disable flag wins over BOOK_RECOMMENDATIONS_TELEMETRY=true.
def _telemetry_disabled() -> bool:
    if os.getenv("BOOK_RECOMMENDATIONS_TELEMETRY", "true").lower() in ("false", "0", "off"):
        return True
...
INTERNAL_RUN = os.getenv("BOOK_RECOMMENDATIONS_INTERNAL", "").lower() in ("1", "true", "yes")


def _init_anonymous_identity():
    """Random installation UUID in ~/.book_recommendations/; created on first run, reset
    ...
    try:
        config_dir = Path.home() / ".book_recommendations"
...
# BOOK_RECOMMENDATIONS_SOURCE, set in install snippets; raw value + low-cardinality bucket.
_KNOWN_SOURCES = {
    "readme", "glama", "mcpso", "pulsemcp", "bookrecs", "setup",
    "cursor_button", "vscode_button", "installer",
}


def _install_source():
    raw = (os.getenv("BOOK_RECOMMENDATIONS_SOURCE") or "").strip().lower()
    if not raw:
        # curl|bash installer writes ~/.book_recommendations/source (env can't survive
        # agent launches); fall back to it so server events carry the bucket.
        try:
            source_file = Path.home() / ".book_recommendations" / "source"
    ...
    def _send():
        try:
            props = {
                ...
                "mcp_server_name": "book-recommendations",
                "$os": platform.system(),
                ...
                try:
                    req = urllib.request.Request(
                        ...
                        headers={
                            ...
                            # the edge
                            "User-Agent": f"book-recommendations/{MCP_SERVER_VERSION}",
                        },
                    )
...
def _track_version_change():
    ...
    try:
        version_file = Path.home() / ".book_recommendations" / "last_run_version"
...
def announce_and_fire_boot_events():
    ...
    if IS_FIRST_INSTALL:
        print(
            "book-recommendations collects anonymous usage telemetry (no PII, no queries, "
            "no paths — see 'Telemetry & Privacy' in the README). "
            "Opt out any time with BOOK_RECOMMENDATIONS_TELEMETRY=false or DO_NOT_TRACK=1.",
            file=sys.stderr,
        )
